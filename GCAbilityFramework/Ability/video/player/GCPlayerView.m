...
+ (instancetype)playerView{
    ...
    static GCPlayerView *player;
    
    static dispatch_once_t onceToken;
    
    dispatch_once(&onceToken, ^{
        
        player = [[GCPlayerView alloc] initWithFrame:CGRectMake(0, 0, _width(), _height())];
    });
    return player;
}
...
- (void)playWith:(NSURL *)url
{
    ...
    [asset loadValuesAsynchronouslyForKeys:[NSArray arrayWithObject:@"tracks"] completionHandler:^{
        ...
        if (status == AVKeyValueStatusLoaded) {
            ...
            GCPlayerItem *item = [[GCPlayerItem alloc] initWithAsset:asset];
            item.observer = self;
...
-(void)createViewsWithFrame:(CGRect)frame
{
    ...
    _topView.backgroundColor = _color(50, 50, 50, .5);
    ...
    _toolView.backgroundColor = _color(50, 50, 50, .5);
    ...
    [_progressSlider setThumbImage:[UIImage getImageWithColor:[UIColor whiteColor] size:CGSizeMake(15, 15) type:ColorImageTypeRound] forState:UIControlStateNormal];
    ...
    if (_volumeView == nil) {
        ...
        for (UIView *view in [_volumeView subviews]){
            if ([view.class.description isEqualToString:@"MPVolumeSlider"]){
                ...
                [self.volumeViewSlider setThumbImage:[UIImage getImageWithColor:[UIColor whiteColor] size:CGSizeMake(10, 10) type:ColorImageTypeRound] forState:UIControlStateNormal];
                break;
            }
        }
    }
...
- (UISlider *)brightnessSlider {
    if (_brightnessSlider == nil) {
        ...
        [_brightnessSlider setThumbImage:[UIImage getImageWithColor:[UIColor whiteColor] size:CGSizeMake(10, 10) type:ColorImageTypeRound] forState:UIControlStateNormal];
    }
